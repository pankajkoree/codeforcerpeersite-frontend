...
export default function UniversityMatchedUserProfile({
  ...
  const [userData, setUserData] = useState<CodeforcesResponse>();
  ...
  // formatting date from both normal as well as unix
  ...
  // for getting country name, to further proceed for country code
  const [countryName, setCountryName] = useState<string>("");
  useEffect(() => {
    if (userData && userData.result && userData.result[0]) {
      setCountryName(userData.result[0].country || "");
    }
  }, [userData]);
